...
import com.iven.musicplayergo.fragments.LLMChatFragment
...
import com.iven.musicplayergo.WakeupIntegration
...
//    private var mLLMChatFragment: LLMChatFragment? = null
    ...
    private val TAG = "WakeupLog"
    private val TAG1 = "WakeupLog"
    private fun logD(msg: String) = Log.d(TAG, msg)
    private fun logE(msg: String) = Log.d(TAG1, msg)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ✅ 注册蓝牙连接监听器
//        BluetoothScoHelper.registerBluetoothReceiver(this)
//
...
//                    VoiceRecognizer.init(context ?: return)
                    ...
                    WakeupIntegration.setViewModel(musicViewModel)
                    logD("[speedtotext] ▶ setViewModel初始化")

                    // 2. 启动前台 Service（保持与唤醒同一个进程）
                    ContextCompat.startForegroundService(
                        this,
                        WakeupService.newIntent(this)
                    )
    ...
    private fun finishSetup(music: MutableList<Music>?) {
        if (!music.isNullOrEmpty()) {
            mMainActivityBinding.loadingProgressBar.handleViewVisibility(show = false)
            logD("[player] ▶ ACTION_PLAYER_READY 广播即将发送")
            LocalBroadcastManager.getInstance(this)
                .sendBroadcast(Intent("ACTION_PLAYER_READY"))
            // Be sure that prefs are initialized
            logD("[player] ✔ ACTION_PLAYER_READY 已发送")
...
//                tab.contentDescription = Theming.getTabAccessibilityText(selectedTab) as CharSequence
//                tab.setContentDescription(Theming.getTabAccessibilityText(selectedTab))
    ...
    private fun initFragmentAt(position: Int): Fragment {
        return when (mGoPreferences.activeTabs.toList()[position]) {
            GoConstants.ARTISTS_TAB -> MusicContainersFragment.newInstance(GoConstants.ARTIST_VIEW)
            GoConstants.ALBUM_TAB -> MusicContainersFragment.newInstance(GoConstants.ALBUM_VIEW)
            GoConstants.SONGS_TAB -> AllMusicFragment.newInstance()
            GoConstants.FOLDERS_TAB -> MusicContainersFragment.newInstance(GoConstants.FOLDER_VIEW)
            GoConstants.LLM_CHAT_TAB -> LLMChatFragment() // ✅ 每次返回新实例，避免重复添加
            else -> SettingsFragment.newInstance()
        }
    }

    private fun handleOnNavigationItemSelected(index: Int): Fragment {
        return initFragmentAt(index) // ✅ 简洁、安全，避免死循环
    }
